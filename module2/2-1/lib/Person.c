...
int8_t pscomp(const Person* const p1, const Person* const p2){
    if(p1 == p2)
        return 0;
    if(!p1 || !p2)
        return -2;
    
    if(!nseq(&p1->name, &p2->name))
        return 1;
    if(!addrSoftEq(&p1->address, &p2->address))
        return 1;
    
    if(p2->emailsCount > p1->emailsCount)
        return 1;
    uint8_t checks = 0;

    for(uint8_t k = 0; k < p2->emailsCount; k++){
        for(uint8_t l = 0; l < p1->emailsCount; l++){
            if(emailEq(&p1->email[k], &p2->email[l])){
                checks++;
                break;
            }
        }
    }
    if(checks != p2->emailsCount)
        return 1;
    
    if(p2->socialsCount > p1->socialsCount)
        return 1;
    
    checks = 0;

    for(uint8_t k = 0; k < p1->socialsCount; k++){
        for(uint8_t l = 0; l < p2->socialsCount; l++){
            if(soseq(&p1->socials[k], &p2->socials[l])){
                checks++;
                break;
            }
        }
    }
    if(checks != p2->socialsCount)
        return 1;
    
    if(p2->phoneCount > p1->phoneCount)
        return 1;
    checks = 0;

    for(uint8_t k = 0; k < p2->phoneCount; k++){
        for(uint8_t l = 0; l < p1->phoneCount; l++){
            if(phEq(&p1->phone[k], &p2->phone[l])){
                checks++;
                break;
            }
        }
    }
    if(checks != p2->phoneCount)
        return 1;

    if(!jseq(&p1->job, &p2->job))
        return 1;
