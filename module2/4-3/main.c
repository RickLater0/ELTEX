...
void editContactMenu(Contacts* book) {
    ...
    printf("1. Изменить ФИО (полностью)\n");
    printf("2. Изменить конкретный телефон (по индексу)\n");
    printf("3. Изменить конкретный email (по индексу)\n");
    printf("4. Изменить адрес\n");
    printf("5. Изменить конкретную соцсеть (по индексу)\n");
    printf("6. Изменить место работы\n");
    printf("7. Заменить весь контакт (полностью)\n");
    printf("---------------------------------\n");
    printf("8. Добавить новый телефон\n");
    printf("9. Удалить телефон (по индексу)\n");
    printf("10. Добавить новый email\n");
    printf("11. Удалить email (по индексу)\n");
    printf("12. Добавить новую соцсеть\n");
    printf("13. Удалить соцсеть (по индексу)\n");
    printf("0. Отмена\n");
    ...
    switch (choice) {
        ...
        case 2: {
            if (p->phoneCount == 0) { printf("[!] У контакта нет телефонов.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс телефона (0..%d): ", p->phoneCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->phoneCount) { printf("[!] Неверный индекс.\n"); return; }
            Phone newPhone;
        ...
        case 3: {
            if (p->emailsCount == 0) { printf("[!] У контакта нет email-ов.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс email (0..%d): ", p->emailsCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->emailsCount) { printf("[!] Неверный индекс.\n"); return; }
            Email newEmail;
        ...
        case 5: {
            if (p->socialsCount == 0) { printf("[!] У контакта нет соцсетей.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс соцсети (0..%d): ", p->socialsCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->socialsCount) { printf("[!] Неверный индекс.\n"); return; }
            Socials newSocial;
        ...
        case 7: {
            ...
            if (!newPerson) { printf("[!] Ошибка создания нового контакта.\n"); return; }
            result = changeContact(book, id, 1, PERSON, *newPerson);
            free(newPerson); // Так как createPersonFromInput делает malloc
            break;
        }
        case 8: {
            if (p->phoneCount >= PHONES_AMOUNT) { printf("[!] Достигнут лимит телефонов (%d).\n", PHONES_AMOUNT); return; }
            Phone newPhone;
            inputSinglePhone(&newPhone);
            result = addPhoneTo(book, id, newPhone);
            break;
        }
        case 9: {
            if (p->phoneCount == 0) { printf("[!] У контакта нет телефонов для удаления.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс удаляемого телефона (0..%d): ", p->phoneCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->phoneCount) { printf("[!] Неверный индекс.\n"); return; }
            result = rmPhoneFrom(book, id, (uint8_t)idx);
            break;
        }
        case 10: {
            if (p->emailsCount >= EMAILS_AMOUNT) { printf("[!] Достигнут лимит email (%d).\n", EMAILS_AMOUNT); return; }
            Email newEmail;
            inputSingleEmail(&newEmail);
            result = addEmailTo(book, id, newEmail);
            break;
        }
        case 11: {
            if (p->emailsCount == 0) { printf("[!] У контакта нет email для удаления.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс удаляемого email (0..%d): ", p->emailsCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->emailsCount) { printf("[!] Неверный индекс.\n"); return; }
            result = rmEmailFrom(book, id, (uint8_t)idx);
            break;
        }
        case 12: {
            if (p->socialsCount >= SOCIALS_AMOUNT) { printf("[!] Достигнут лимит соцсетей (%d).\n", SOCIALS_AMOUNT); return; }
            Socials newSocial;
            inputSingleSocial(&newSocial);
            result = addSocialTo(book, id, newSocial);
            break;
        }
        case 13: {
            if (p->socialsCount == 0) { printf("[!] У контакта нет соцсетей для удаления.\n"); return; }
            int idx;
            snprintf(prompt, sizeof(prompt), "Введите индекс удаляемой соцсети (0..%d): ", p->socialsCount - 1);
            if (!safeInputInt(prompt, &idx) || idx < 0 || idx >= p->socialsCount) { printf("[!] Неверный индекс.\n"); return; }
            result = rmSocialFrom(book, id, (uint8_t)idx);
            break;
        }
        default:
    ...
    if (result >= 0) {
        printf("[+] Изменения успешно применены.\n");
        printf("\n--- Обновлённые данные ---\n");
        prcont(book, id);
    } else {
